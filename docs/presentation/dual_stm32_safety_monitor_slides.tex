...
\begin{frame}{设计目标 / Design Goals}
  \begin{tabularx}{\textwidth}{@{}p{0.23\textwidth}YY@{}}
    ...
    目标 & 设计要求 & 对应实现 \\
    \midrule
    稳定采集 & 每秒发帧，MQ/雨量/热敏每帧采样，DHT11 安全间隔刷新 & \code{SensorNode::run()} + 整数滑动平均 \\
    可靠通信 & 串口帧能识别错位和噪声 & \code{AA 55} 帧头 + 长度 + checksum \\
    本地报警 & 无需云端即可及时提示风险 & 蜂鸣器 + OLED 状态页 + Web Serial 看板 \\
    易演示 & 可观察、可断线恢复、可用按键交互 & USART1 日志、NODE LOST、K1/K2、PB0/PB1 \\
    ...
    \begin{column}{0.5\textwidth}
      \begin{block}{板 B：MONITOR}
        \begin{itemize}
          ...
          \item 蜂鸣器：声音报警
          \item K1/K2：切页、静音
          \item PB0/PB1：选择传感器、调整阈值档位
          \item W25Q64：可选历史记录
          \item USART1：JSON Lines 看板输出
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{板载资源约束 / Board Resource Constraints}
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.16\textwidth}p{0.30\textwidth}Y@{}}
    ...
    引脚 & 板载连接 & 本项目处理方式 \\
    \midrule
    PA9 / PA10 & USB 转串口参考通道 & 保留 USART1 做 \code{printf}/JSON 调试 \\
    PA11 / PA12 & USB Device & 保留，不分配外设 \\
    PA13 / PA14 & SWD 下载调试 & 保留给调试器 \\
    PA1 / PA2 / PA3 & 旧演示 RGB 引脚 & 当前应用源码未使用 \\
    PA0 / PC13 & 板载 K1 / K2 & 板 B 用于页面与静音 \\
    PB0 / PB1 & 外接低有效按键 & 板 B 用于选择传感器与调整 5 档阈值 \\
    PB10 / PB11 & 普通可用 IO / USART3 & 用作双板通信链路 \\
    \bottomrule
  \end{tabularx}
  \vspace{0.5em}
  \begin{alertblock}{关键取舍}
    USART1 保留给日志和 JSON Lines；板间通信固定使用 USART3 PB10/PB11，避免与调试通道混用。
  \end{alertblock}
\end{frame}
...
\begin{frame}{总体架构 / Overall Architecture}
  ...
  \begin{tikzpicture}[node distance=6mm and 10mm, font=\scriptsize]
    ...
    \node[greenbox, below=of mq2] (rain) {Rain SIG\\PA6};
    \node[greenbox, below=of rain] (therm) {Therm AO/DO\\PA7/PB9};
    \node[greenbox, below=of therm] (flame) {Flame DO\\PB13};
    ...
    \node[amberbox, below=of keys] (thresholdkeys) {Threshold Keys\\PB0/PB1};
    \node[amberbox, below=of thresholdkeys] (json) {JSON Lines\\USART1};

    \draw[arrow] (dht.east) -- (a.west);
    \draw[arrow] (mq135.east) -- (a.west);
    \draw[arrow] (mq2.east) -- (a.west);
    \draw[arrow] (rain.east) -- (a.west);
    \draw[arrow] (therm.east) -- (a.west);
    ...
    \draw[arrow] (b.east) |- (thresholdkeys.west);
    \draw[arrow] (b.east) |- (json.west);
  \end{tikzpicture}%
  }
\end{frame}
...
\begin{frame}[fragile]{双固件构建方式 / One Source, Two Images}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      \begin{block}{核心思想}
        同一份 \code{Core/Src/main.cpp + App/} 通过 \code{APP_NODE_ROLE} 生成两个不同固件：
        \begin{itemize}
...
\begin{frame}{串口数据帧协议 / USART3 Frame Protocol}
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.10\textwidth}p{0.18\textwidth}Y@{}}
    ...
    2 & \code{LEN} & Payload length, fixed to 18 for v2 \\
    3 & \code{VER} & Protocol version, fixed to 2 \\
    4--5 & \code{TEMP/HUMI} & DHT11 temperature and humidity \\
    6--15 & \code{MQ/RAIN/THERM} & ADC readings and thermistor 0.1 deg C value \\
    16--18 & \code{FLAGS} & Flame, rain wet, thermistor hot \\
    19 & \code{SEQ} & Rolling frame number \\
    20 & \code{STATUS} & DHT/rain/thermistor status bits \\
    21 & \code{CHECKSUM} & Low 8 bits of LEN + payload \\
    \bottomrule
  \end{tabularx}
  \vspace{0.4em}
  \begin{block}{核心函数}
    \code{FrameCodec::encode()} 负责结构体转字节帧，\code{FrameCodec::decode()} 负责帧头、长度、校验和验证。
  \end{block}
\end{frame}
...
\begin{frame}{板 B 协作式调度 / Monitor Scheduler}
  \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}p{0.18\textwidth}Y@{}}
    ...
    任务 & 周期 & 函数与作用 \\
    \midrule
    串口解析 & 每轮循环 & \code{MonitorNode::processRx()}，尽快消化接收缓冲 \\
    按键扫描 & 每轮循环 & \code{MonitorNode::updateButtons()}，保证 K1/K2 与 PB0/PB1 响应 \\
    报警输出 & 100 ms & \code{MonitorNode::updateAlarm()}，更新 LED 和蜂鸣节奏 \\
    OLED 刷新 & 300 ms & \code{MonitorNode::updateDisplay()}，控制刷新负担 \\
    Flash 记录 & 10 s & \code{W25q64FlashLogger::logFrame()}，降低写入频率 \\
    \bottomrule
  \end{tabularx}
  \vspace{0.5em}
...
\begin{frame}{OLED 页面设计 / OLED UI}
  ...
  \end{columns}
  \vspace{0.4em}
  \small K1 切换页面，K2 短按静音；PB0 选择 MQ135/MQ2/雨量/热敏，PB1 调整当前传感器 5 档阈值。
\end{frame}
...
\begin{frame}{按键交互 / Button Interaction}
  \begin{tabularx}{\textwidth}{@{}p{0.18\textwidth}p{0.30\textwidth}Y@{}}
    ...
    按键 & 操作 & 系统反馈 \\
    \midrule
    K1 & 短按 & \code{page_} 翻转，OLED 在监测页与参数页之间切换 \\
    K2 & 短按 & \code{mute_until_ms_ = now + mute_time_ms}，蜂鸣器静音 60 秒 \\
    PB0 & 短按 & 切换当前调节传感器：MQ135、MQ2、雨量、热敏 \\
    PB1 & 短按 & 当前传感器在 \code{0..4} 档循环，OLED 显示为 \code{1/5..5/5} \\
    \bottomrule
  \end{tabularx}
  \vspace{0.5em}
  \begin{block}{边沿检测}
    K1/K2 使用上一轮电平做边沿检测；PB0/PB1 使用约 50 ms 软件防抖，只在稳定按下边沿触发动作。
  \end{block}
\end{frame}
...
\begin{frame}{可选 Flash 记录 / Optional W25Q64 Logging}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.53\textwidth}
      \begin{block}{初始化策略}
        \begin{itemize}
          \item \code{MX_SPI2_Init()} 启用 HAL SPI2。
          \item SPI2 使用 PB13/PB14/PB15，PB12 作为 GPIO CS。
...
\begin{frame}{JSON Schema v2：阈值字段 / Threshold Fields}
  \small
  \begin{tabularx}{\textwidth}{@{}p{0.33\textwidth}Y@{}}
    \toprule
    字段 & 含义 \\
    \midrule
    \code{selectedThresholdSensor} & 当前调节项：0=MQ135，1=MQ2，2=RAIN，3=THERM \\
    \code{thresholdAirLevel} 等 & 四路档位，固件值为 0..4，OLED 显示为 1/5..5/5 \\
    \code{thresholdAirWarn} & MQ135 当前空气质量预警 ADC 阈值 \\
    \code{thresholdSmokeWarn/Danger} & MQ2 当前预警/危险 ADC 阈值 \\
    \code{thresholdRainWet} & 雨量当前湿态 ADC 阈值 \\
    \code{thresholdThermWarn/DangerC10} & 热敏当前预警/危险阈值，单位 0.1 摄氏度 \\
    \code{thresholdProfile} & legacy 兼容：0 默认，1 旧灵敏，2 旧宽松，255 混合/自定义 \\
    \bottomrule
  \end{tabularx}
\end{frame}
...
\begin{frame}{关键全局状态 / Shared State Variables}
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.30\textwidth}p{0.25\textwidth}Y@{}}
    ...
    变量 & 写入者 & 读取者与作用 \\
    \midrule
    \code{latest_frame_} & \code{processRx()} & OLED、报警、Flash 使用的最新合法数据 \\
    \code{last_rx_ms_} & \code{processRx()} & \code{evaluateAlarm()} 判断离线 \\
    \code{page_} & \code{updateButtons()} & OLED 页面选择 \\
    \code{threshold_levels_} & PB1 / \code{thresholdsFromLevels()} & 保存四路独立阈值档位并生成报警阈值 \\
    \code{selected_threshold_sensor_} & PB0 / \code{updateButtons()} & 选择当前调节的传感器 \\
    \code{mute_until_ms_} & \code{updateButtons()} & 控制蜂鸣器静音截止时间 \\
    \code{flash_.present()} & \code{W25Q64 init()} & 控制是否启用日志记录 \\
...
\begin{frame}{异常处理 / Fault Handling}
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.25\textwidth}p{0.30\textwidth}Y@{}}
    ...
    异常 & 检测位置 & 处理方式 \\
    \midrule
    DHT11 读取失败 & \code{Dht11::read()} & 设置 \code{STATUS_DHT_ERROR}，继续发帧 \\
    串口噪声或错位 & \code{FrameCodec::decode()} & 丢弃坏帧，重新寻找 \code{AA 55} \\
    接收缓冲满 & \code{USART3_IRQHandler()} & 丢弃最新字节，避免 ISR 阻塞 \\
    板 A 离线 & \code{evaluateAlarm()} & OLED 显示 NODE LOST，蜂鸣器慢速提示 \\
    Flash 未接入 & \code{W25q64FlashLogger::init()} & \code{flash_.present()=0}，主功能不受影响 \\
...
\begin{frame}{测试计划 / Test Plan}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}
      \begin{block}{单节点测试}
        \begin{itemize}
          \item 板 A：DHT11、MQ135、MQ2、雨量、热敏、火焰模块。
          \item 板 B：OLED、蜂鸣器、K1/K2、PB0/PB1、W25Q64、JSON Lines。
...
\begin{frame}{演示流程 / Demonstration Flow}
  \begin{enumerate}
    \item 烧录 \code{Env-Monitor_sensor.hex} 到板 A，烧录 \code{Env-Monitor_monitor.hex} 到板 B。
    \item 连接 USART3：A PB10 到 B PB11，B PB10 到 A PB11，并共地。
    \item 打开两个 USB 转串口，确认板 A \code{[SENSOR]} 日志和板 B \code{[MONITOR] rx v2} 日志。
    \item 展示 OLED 监测页和参数页，按 K1 切换页面。
    \item 触发烟雾、雨量、热敏或火焰模块，观察 OLED、看板风险和蜂鸣器变化。
    \item 短按 K2 展示静音；按 PB0 选择传感器，按 PB1 展示 5 档阈值调整。
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{文档结构}
        \begin{itemize}
          ...
          \item \code{WIRING.md}
          \item \code{docs/FUNCTION_GUIDE.en.md} / \code{.zh-CN.md}
          \item \code{docs/FUNCTION_DESIGN_WALKTHROUGH.*.md}
          \item \code{docs/PROJECT_STRUCTURE.en.md} / \code{.zh-CN.md}
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{当前限制 / Current Limitations}
  \begin{tabularx}{\textwidth}{@{}p{0.30\textwidth}Y@{}}
    ...
    Flash 记录为演示型 & 第 0 扇区保存元数据，sector 1+ 环形记录，不是工业级 wear leveling \\
    \bottomrule
  \end{tabularx}
\end{frame}
...
\begin{frame}{可扩展方向 / Future Work}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.46\textwidth}
      \begin{block}{工程改进}
        \begin{itemize}
          \item 拆分 \code{main.cpp} 与 \code{App/} 为 protocol、sensor、monitor 模块。
          \item 增加 CRC8/CRC16。
          \item 更完整的按键消抖。
          \item 增加单元测试或串口帧模拟器。
        \end{itemize}
...
\begin{frame}{总结 / Summary}
  \begin{block}{项目成果}
    本系统完成了双 STM32 节点分工、USART3 数据通信、本地 OLED 显示、蜂鸣器报警、按键交互、JSON 看板和可选 Flash 记录。
  \end{block}
  \begin{block}{工程价值}
