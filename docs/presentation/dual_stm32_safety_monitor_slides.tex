...
\begin{frame}{总体架构 / Overall Architecture}
  ...
  \begin{tikzpicture}[node distance=8mm and 10mm, font=\small]
    ...
    \node[amberbox, below=of keys] (json) {JSON Lines\\USART1};
...
\begin{frame}{板 B 协作式调度 / Monitor Scheduler}
  \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}p{0.18\textwidth}Y@{}}
    ...
    按键扫描 & 每轮循环 & \code{MonitorNode::updateButtons()}，保证 K1/K2 响应 \\
...
\begin{frame}{按键交互 / Button Interaction}
  \begin{tabularx}{\textwidth}{@{}p{0.18\textwidth}p{0.30\textwidth}Y@{}}
    ...
    K2 & 长按 & \code{threshold_profile_} 递增，切换阈值档位 \\
    \bottomrule
  \end{tabularx}
  \vspace{0.5em}
  \begin{block}{边沿检测}
...
\begin{frame}{关键全局状态 / Shared State Variables}
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}p{0.24\textwidth}Y@{}}
    ...
    变量 & 写入者 & 读取者与作用 \\
    \midrule
    \code{latest_frame_} & \code{MonitorNode::processRx()} & OLED、报警、Flash 使用的最新合法数据 \\
    \code{last_rx_ms_} & \code{MonitorNode::processRx()} & \code{evaluateAlarm()} 判断离线 \\
    \code{page_} & \code{MonitorNode::updateButtons()} & OLED 页面选择 \\
    \code{threshold_profile_} & \code{MonitorNode::updateButtons()} & 影响报警阈值与显示参数 \\
    \code{mute_until_ms_} & \code{MonitorNode::updateButtons()} & 控制蜂鸣器静音截止时间 \\
    \code{flash_.present()} & \code{W25q64FlashLogger::init()} & 控制是否启用日志记录 \\
    ...
    \begin{column}{0.5\textwidth}
      \begin{block}{单节点测试}
        \begin{itemize}
          ...
          \item 板 B：OLED、蜂鸣器、K1/K2、W25Q64、JSON Lines。
