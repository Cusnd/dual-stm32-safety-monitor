...
\begin{frame}{总体架构 / Overall Architecture}
  ...
  \begin{tikzpicture}[node distance=8mm and 10mm, font=\small]
    ...
    \node[amberbox, below=of keys] (ergb) {WS2813E\\PA6};
...
\begin{frame}{关键全局状态 / Shared State Variables}
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}p{0.24\textwidth}Y@{}}
    ...
    \code{last_rx_ms_} & \code{MonitorNode::processRx()} & \code{MonitorNode::nodeLost()} 判断离线 \\
    \code{page_} & \code{MonitorNode::updateButtons()} & OLED 页面选择 \\
    \code{threshold_profile_} & \code{MonitorNode::updateButtons()} & 影响报警阈值与显示参数 \\
    \code{g_mute_until_ms} & \code{MonitorNode::updateButtons()} & 控制蜂鸣器静音截止时间 \\
...
\begin{frame}{异常处理 / Fault Handling}
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.25\textwidth}p{0.30\textwidth}Y@{}}
    ...
    板 A 离线 & \code{MonitorNode::nodeLost()} & OLED 显示 NODE LOST，蓝灯慢蜂鸣 \\
    ...
    \begin{column}{0.5\textwidth}
      \begin{block}{单节点测试}
        \begin{itemize}
          ...
          \item 板 B：OLED、RGB、WS2813E、蜂鸣器、K1/K2。
...
\begin{frame}{演示流程 / Demonstration Flow}
  \begin{enumerate}
    ...
    \item 触发烟雾、雨量、热敏或火焰模块，观察状态灯和蜂鸣器变化。
