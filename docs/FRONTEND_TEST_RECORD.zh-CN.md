...
- 前端：静态 Web Serial 页面、JSON Lines 解析器、动态模拟串口、AI 本地洞察、DeepSeek 后端代理调用和本地兜底 provider
- 硬件/真实 LLM：未连接实物或未观察到真实后端响应时只记录为待测，不伪造结果
...
| 前端全量单测 | `npm --prefix frontend test` | 通过 | 21 项通过，0 项失败；覆盖 parser、analysis、replay source、DeepSeek proxy 请求、纯文本响应和本地兜底 |
...
| DeepSeek 前端代理逻辑 | 默认 DeepSeek 模式下输入“现在安全吗？” | 通过 | 静态服务无 `/api/ai/chat` 时自动回落到“本地规则兜底”；单测验证请求体包含模型、消息、快照且不包含 API key |
...
| DeepSeek V4-flash 真实后端联调 | 后端代理 `/api/ai/chat` | 待测 | 前端已完成代理调用和兜底逻辑；需要后端服务运行并观察真实 DeepSeek 响应 |
...
当前未写入任何硬件或真实 DeepSeek 通过结论。只有实际连接板 B、选择 USB 转串口并观察到页面实时刷新后，才能把硬件联调结果改为通过；只有后端代理运行并观察到真实 DeepSeek 响应后，才能把真实后端联调结果改为通过。
