...
- 前端：静态 Web Serial 页面、JSON Lines 解析器、动态模拟串口、AI 本地洞察、DeepSeek 浏览器直连、可选后端代理调用和本地兜底 provider
...
| 前端全量单测 | `npm --prefix frontend test` | 通过 | 23 项通过，0 项失败；覆盖 parser、analysis、replay source、DeepSeek 直连请求、proxy 请求、纯文本响应和本地兜底 |
