...
- **Board B: MONITOR node**  
  Uses a stream decoder to recover USART3 frames, evaluates warning/danger/node-lost states, refreshes SSD1306 OLED pages, drives the buzzer, handles K1/K2, optionally logs fixed records to W25Q64, and prints JSON Lines on USART1.
...
- `W25q64FlashLogger` is optional and non-blocking: sector 0 stores cursor metadata, sector 1 through 8 MB stores 32-byte circular records.
- The monitor prints JSON schema v2 fields for rain, thermistor, threshold profile, mute state, flash presence, and flash record count.
...
- [docs/FUNCTION_GUIDE.en.md](docs/FUNCTION_GUIDE.en.md) / [中文](docs/FUNCTION_GUIDE.zh-CN.md): function-level reading guide.
...
- [docs/PROJECT_STRUCTURE.en.md](docs/PROJECT_STRUCTURE.en.md) / [中文](docs/PROJECT_STRUCTURE.zh-CN.md): repository layout and edit guide.
...
The monitor converts accepted frames into JSON schema v2. The current required v2 extension fields are `rainRaw`, `thermRaw`, `thermC10`, `rainWet`, `thermHot`, and `flashRecords`. The firmware may also print `externalRgb` as a legacy placeholder; the dashboard must not treat it as a required active output.
...
5. Validate `normal`, `warn`, `danger`, and frontend stale/node-lost behavior on OLED, buzzer, dashboard status, and W25Q64 record count.
